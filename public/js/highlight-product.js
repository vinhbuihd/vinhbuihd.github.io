            ...
            <div class="card-content">
                ...
                <div class="card-content-right d-flex align-items-center justify-content-between">
                    ...
                    <div onclick="clickBuy(${
                      product.id
                    })" class="card-cart d-flex align-items-center justify-content-center"><i class="fa-solid fa-basket-shopping"></i></div>
                </div>
            </div>
        </div>
        `;
    ...
    popup.innerHTML = `
        <div class="popup-box">
            ...
            <div class="popup-bottom mt-16 d-flex align-items-center justify-content-center flex-column">
                ...
                <button class="btn">Thêm vào giỏ hàng : <span>${(
                  product.price.S * product.quantity
                ).toLocaleString()}</span> đ</button>
            </div>
        </div>
        `;
  }
      ...
      cardSideElement.innerHTML += `
                <div class="cart-item d-flex" dataset=${product.code}>
                    ...
                    <div class="cart-item-info">
                        ...
                        <div class="size d-flex align-items-center">Size ${
                          product.size
                        }<span>x ${product.quantity}</span>
                            <div class="edit" onclick='editItem(${
                              product.code
                    ...
                    <div class="d-flex align-items-center" >
                        <div class="cart-item-sub-price">
                            <span>${(
                              product.quantity * product.sizePrice
                            ).toLocaleString()}</span> đ
                        </div>
                    </div>
...
function addToCart(product) {
  ...
  const popupQuantity = popup.querySelector(
    ".popup-bottom .cart-item-quantity"
  );
  ...
  radios.forEach((radio) => {
    radio.onchange = function () {
      popupBottomPrice.innerHTML = (
        product.price[radio.value] * product.quantity
      ).toLocaleString();
    };
  });
  ...
  addToCart.addEventListener("click", function () {
    ...
    if (cartList.length == 0) {
      ...
      let isEqual = cartList.find(
        (product) =>
          newItem.id == product.id &&
          newItem.note == product.note &&
